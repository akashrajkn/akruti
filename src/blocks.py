...
class L_MSVAE(nn.Module):
    '''
    Preliminaries: Labeled MSVAE
    '''
    def __init__(self, vocab_size, emb_dim, tag_embedding, z_dim, h_dim, device, max_len, padding_idx=None):
        super(L_MSVAE, self).__init__()

        self.input_dim     = vocab_size
        self.output_dim    = vocab_size
        self.tag_embedding = tag_embedding
        self.emb_dim       = emb_dim
        self.tag_emb_dim   = tag_embedding.emb_dim
        self.z_dim         = z_dim
        self.h_dim         = h_dim
        self.device        = device
        self.vocab_size    = vocab_size
        ...
        self.embedding     = nn.Embedding(self.input_dim, emb_dim, padding_idx=padding_idx)
        self.rnn_enc       = nn.GRU(emb_dim, h_dim, bidirectional=True)
        self.rnn_dec       = nn.GRU(emb_dim + z_dim + self.tag_emb_dim, h_dim * 2)
        self.fc_mu         = nn.Linear(h_dim * 2, z_dim)
        self.fc_sigma      = nn.Linear(h_dim * 2, z_dim)
        self.out           = nn.Linear(h_dim * 2, self.output_dim)
    ...
    def decoder(self, o, h, z, tag_embeds):
        ...
        rnn_dec_input  = torch.cat((embedded, z, weighted), dim=2)
        output, hidden = self.rnn_dec(rnn_dec_input, h.unsqueeze(0))
        output         = output.squeeze(0)
        output         = self.out(output)
    ...
    def forward(self, x, y):
        ...
        tag_embeds       = self.tag_embedding(y)
        h, z_mu, z_sigma = self.encoder(x)
        z                = self.reparameterize(z_mu, z_sigma)
        o                = x[0, :]

        for step in range(1, self.max_len):
            o, h          = self.decoder(o, h, z, tag_embeds)
            outputs[step] = o
            o             = o.max(1)[1]
